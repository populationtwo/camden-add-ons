...
/**
 * Foundation Shortcode forked from https://github.com/wearerequired/required-foundation
 */
class REQ_Alertbox {
	...
	/**
	 * Registers the [callout] shortcode.
	 *
	 * @since  0.1.0
	 * @access public
	 * @return void
	...
	public function register_shortcode() {
		add_shortcode( 'callout', array( &$this, 'do_shortcode' ) );
	}
...
class REQ_Orbit {
	...
	public function do_shortcode( $attr, $content = null ) {
		...
		/* Let the magic happen */
		$output = '<div class="camden-orbit orbit" role="region" id="req-orbit-' . $id . '" data-orbit data-options="animInFromLeft:fade-in;animInFromRight:fade-in; animOutToLeft:fade-out; animOutToRight:fade-out;">';
		...
		foreach ( $attachments as $id => $attachment ) {
			...
			if ( trim( $attachment->post_excerpt ) ) {
				...
				$caption      = '<figcaption class="orbit-caption">' . wptexturize( $attachment->post_excerpt ) . '</figcaption>';
			}

			/* Generate final item output */
			$output .= '<li class="orbit-slide"><img class="orbit-image" src="' . esc_url( $img_src[0] ) . '" ' . $data_caption . ' alt=""/>';
			$output .= $caption;
			$output .= '</li>';
		}
		...
		foreach ( $attachments as $id => $attachment ) {
			...
			/* Generate final item output */
			if ( $slideIndex == 0 ) {
				$output .= '<button class="is-active" data-slide="' . $slideIndex . '"><span class="show-for-sr">' . $slideIndex . '</span><span class="show-for-sr">' . __( 'Current Slide', 'camden' ) . '</span></button>
';
			} else {
				$output .= '<button data-slide="' . $slideIndex . '"><span class="show-for-sr">' . $slideIndex . '</span></button>
';
			};
			$slideIndex ++;
...
class REQ_Reveal {
	...
	public function do_shortcode( $attr, $content = null ) {
		...
		$output = '<a class="' . $link_class . '" data-open="' . esc_attr( $modal_unique_id ) . '">' . esc_attr( $attr['link'] ) . '</a>';

		$footer_output = '<div id="' . esc_attr( $modal_unique_id ) . '" class="' . $reveal_class . ' " data-reveal data-animation-in="fade-in fast" >';
		$footer_output .= apply_filters( 'req_reveal_content', $content );
		$footer_output .= '<button class="close-button" data-close aria-label="Close modal" type="button"><span aria-hidden="true">&times;</span></button>';
		$footer_output .= '</div>';
...
class REQ_Tooltip {
	...
	public function do_shortcode( $attr, $content = null ) {
		...
		/* Create our output */
		$output = '<span class="' . $tooltip_class . '"' . $title . $width . ' data-tooltip aria-haspopup="true" data-disable-hover="false" tabindex="1">' . apply_filters( 'req_tooltip_content', $content ) . '</span>';
...
function camden_foundation_tabs( $params, $content = null ) {
	...
	if ( isset( $foundation_tabs[ $id ]['tabs'] ) && is_array( $foundation_tabs[ $id ]['tabs'] )
		...
		$scontent .= '<ul class="tabs" data-tabs id="ft-' . $id . '">';
		$scontent .= implode( '', $foundation_tabs[ $id ]['tabs'] );
		$scontent .= '</ul>';
		$scontent .= '<div class="tabs-content" data-tabs-content="ft-' . $id . '">';
...
add_shortcode( 'tabs', 'camden_foundation_tabs' );


function camden_foundation_tab( $params, $content = null ) {
	extract(
		shortcode_atts(
			array(
				'title'  => 'title',
	...
	$foundation_tabs[ $index ]['tabs'][] = '<li class="tabs-title ' . $active . '"><a href="#' . $pane_id . '">' . $title
	                                       . '</a></li>';
	$foundation_tabs[ $index ]['panes'][]
	                                     =
		'<div class="tabs-panel ' . $active . '" id="' . $pane_id . '"><p>' . do_shortcode( trim( $content ) ) . '</p></div>';

}

add_shortcode( 'tab', 'camden_foundation_tab' );


function camden_foundation_vtabs( $params, $content = null ) {
	global $foundation_tabs;
	extract(
		shortcode_atts(
			array(
	...
	if ( isset( $foundation_tabs[ $id ]['tabs'] ) && is_array( $foundation_tabs[ $id ]['tabs'] )
		...
		$scontent .= '<div class="medium-3 columns"><ul class="tabs vertical" data-tabs id="fvt-' . $id . '" >';
		$scontent .= implode( '', $foundation_tabs[ $id ]['tabs'] );
		$scontent .= '</ul></div>';
		$scontent .= '<div class="medium-9 columns"><div class="tabs-content vertical" data-tabs-content="fvt-' . $id . '" >';
		$scontent .= implode( '', $foundation_tabs[ $id ]['panes'] );
		$scontent .= '</div></div>';
	}
	$scontent .= '</div></div>';
...
add_shortcode( 'vtabs', 'camden_foundation_vtabs' );


function camden_foundation_vtab( $params, $content = null ) {
	extract(
		shortcode_atts(
			array(
				'title'  => 'title',
...
add_shortcode( 'vtab', 'camden_foundation_vtab' );
...
function camden_foundation_accordion( $params, $content = null ) {
...
function camden_foundation_accordion_pane( $params, $content = null ) {
	global $foundation_accordion;
	extract(
		shortcode_atts(
			array(
	...
	$pane_id = 'camden-tabpane-' . $index . '-' . count( $foundation_accordion[ $index ]['tabs'] );

	$foundation_accordion[ $index ]['tabs'][] = '<li class="accordion-item ' . $active . '" data-accordion-item><a href="#' . $pane_id . '" class="accordion-title">' . $title
	                                            . '</a><div class="accordion-content" id="' . $pane_id . '" data-tab-content>'
	                                            . do_shortcode( trim( $content ) ) . '</div></li>';
